...
export function IconButton({children, ...props}) {
  return <Button className="icon" {...props}>{children}</Button>
}
...
export class MakeToggleGroup extends React.PureComponent {
  ...
  static makeButton(buttons, disabled, choice) {
    ...
    return <Toggle key={choice} className="toggle icon" disabled={isDisabled} value={choice} tooltip={tooltip}>{icon}</Toggle>
  }
}
...
class MenuArrow extends React.PureComponent {
  render() {
    return <Menu.Arrow className="Arrow" {...this.props}>
      <svg width="20" height="10" viewBox="0 0 20 10" fill="none" {...this.props}>
        <path
